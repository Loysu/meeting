...
class ProfileFilter(filters.FilterSet):
    ...
    first_name = filters.CharFilter(
        field_name="user__first_name", label="Имя", lookup_expr="icontains"
    )
    last_name = filters.CharFilter(
        field_name="user__last_name", label="Фамилия", lookup_expr="icontains"
    )
    distance = filters.NumberFilter(
        label="Расстояние (в километрах)", method="calc_distance"
    )
    ...
    def calc_distance(self, queryset, name, value):
        ...
        if self.request.user.is_authenticated:
            ...
            for profile in queryset.all():
                ...
                distance_between_two_profiles = 6371 * acos(
                    sin(radians(request_profile.latitude))
                    * sin(radians(profile.latitude))
                    + cos(radians(request_profile.latitude))
                    * cos(radians(profile.latitude))
                    * cos(radians(request_profile.longitude - profile.longitude))
                )
